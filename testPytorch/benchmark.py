...
def test(epoch):
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}% New:{:.0f}%)\n'.format(
        test_loss, correct, len(test_loader.dataset),
        100. * correct / len(test_loader.dataset), running_accuracy / count))
...
prof.export_chrome_trace("./runs/benchmark/profiler/benchmark_training_profiler.json")
...
if use_gpu:
    ...
    print(prof.key_averages(group_by_input_shape=True).table(sort_by="cpu_time_total", row_limit=10))
prof.export_chrome_trace("./runs/benchmark/profiler/benchmark_inference_profiler.json")
